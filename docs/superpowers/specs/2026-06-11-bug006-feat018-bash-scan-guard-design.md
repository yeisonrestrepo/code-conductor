...
- Token check: each entry is matched against the **comment-stripped** command string (quotes are preserved) using boundary delimiters (whitespace, `;`, `|`, `(`, `)`, `{`, `[`, `}`, `]`, `'`, `"`) on both sides. The forward slash is **not** a delimiter. When an entry ends with `/`, alphanumeric characters, `_`, `-`, and `.` are additionally permitted on the right side, so `"docs/"` matches `docs/file.txt` and `docs/sub/` without treating the following filename as a substring violation. A substring-only match is not sufficient.
...
- [ ] Pattern 5 does not block a reading utility whose command substitution is followed by a `/`-prefixed static path suffix with no glob or expansion characters, even when quotation marks are adjacent to the closing `)` or backtick (e.g., `cat "$(git rev-parse --show-toplevel)"/package.json` and `cat "$(git rev-parse --show-toplevel)/package.json"` both pass through).
- [ ] Pattern 6 recursive flag corrected to `-r`, `-R`, or `--recursive` (not `--all`); Pattern 6 isolates the pattern argument correctly when `-e` is used — any match-all `-e` expression fires the rule; `-f` (file-sourced patterns) is out of scope and does not trigger.
- [ ] Allowlist entries ending with `/` permit alphanumeric, `_`, `-`, and `.` characters on the right boundary, so `"docs/"` matches `docs/file.txt` without treating the filename as a substring violation.
...
- [ ] The three-state scanner advances the loop index by 2 (`i += 2`) when processing a backslash escape, ensuring the escaped character is not re-evaluated in the next iteration.
