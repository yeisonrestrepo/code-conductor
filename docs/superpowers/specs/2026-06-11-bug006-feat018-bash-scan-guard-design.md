...
4. Guard 3 preprocesses the command string in two sequential normalization steps before any pattern evaluation:
   ...
   - **4b — Comment stripping:** Remove everything from the first unquoted, non-backslash-escaped `#` to end-of-line. A `\#` sequence is a literal hash and does not begin a comment (e.g., a regex argument of `\#foo` is preserved intact). **Tokenization strategy (pure Bash):** Use a three-state character scanner — UNQUOTED, SINGLE_QUOTED, DOUBLE_QUOTED — implemented as a `while` loop over `${cmd:i:1}` character indices. Rules: entering `'` from UNQUOTED transitions to SINGLE_QUOTED (no escaping is possible inside single quotes; only a closing `'` exits); entering `"` from UNQUOTED transitions to DOUBLE_QUOTED (a `\` in this state escapes the next character); a `#` encountered in UNQUOTED state that is not preceded by `\` begins a comment and everything from that index to end-of-line is removed. This produces the **comment-stripped string**, which is the base for all subsequent steps.
...
2. **Command execution position** — the name must appear as an executed command token: at the start of the command string, or immediately following a shell operator (`|`, `&&`, `||`, `;`, `;;`, `$(`, `<(`, `>(`, `` ` ``, `{`, `[`, or newline), or immediately following the keyword `then`. A binary name appearing as a path component (preceded by `/`) or as a plain argument is not in command position and must not trigger a block (e.g., `grep -r pattern /path/to/find/results` must not block on `find`).
...
| 9 | Shell loop + reading | `for f in *.ts; do cat $f; done`, `while true; do less $f; done`, `until false; do grep -r . ; done` | Command contains `for`, `while`, or `until` as an unquoted shell keyword co-occurring with any reading utility; "unquoted" is determined using the same three-state scanner defined in Step 4b — the keyword is only a shell keyword when encountered in UNQUOTED state (e.g., `echo "while true"` encounters `while` in DOUBLE_QUOTED state → must not fire) |
...
- [ ] Command execution position recognises `{`, `[`, backtick `` ` ``, `<(`, `>(`, `;;`, and `then` as valid preceding operators alongside `|`, `&&`, `||`, `;`, `$(`, and newline.
...
- [ ] Unquoted-hash detection (Step 4b) and unquoted-keyword detection (Pattern 9) both use the three-state scanner (UNQUOTED / SINGLE_QUOTED / DOUBLE_QUOTED) implemented as a character-by-character Bash loop.
