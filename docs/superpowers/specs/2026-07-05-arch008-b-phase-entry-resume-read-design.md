...
4. **Bind by precedence:**
   ...
   - **DB miss/degrade/bypass** → fall back to the handoff file. First test existence (`existsSync(HANDOFF)`); if absent → miss. If present, read its bytes **into memory first** inside a `try/catch`:
     ...
     - read succeeds → **empty check first:** if the captured content is empty or whitespace-only (`content.trim() === ''` — the signature of a truncated/torn or not-yet-flushed write, not a corrupt authoritative handoff), **degrade to a clean miss** (exit 3), best-effort delete the useless file, and do **not** halt. Trace: `resume: file-empty degrade`. (This runs before validation precisely so an empty file never reaches the validator and never trips the halt path.)
     - non-empty, then validate:
       - **structurally invalid** (readable, non-empty, but malformed JSON / bad schema / unknown version) → **halt** (exit 4), leave the file on disk. Trace: `resume: file-invalid halt`.
...
- **Handoff file present but empty / whitespace-only** (zero bytes or only whitespace — a torn or not-yet-flushed write) → **degrade to a clean miss** (exit 3), best-effort delete the useless file. Checked *before* validation so an empty file never reaches the validator's `empty file` rejection and never halts; an incomplete write must not trap the user.
...
- [ ] The Solution paragraph says "full-length git hash (40-char SHA-1 or 64-char SHA-256)", not "full-40".
- [ ] An empty / whitespace-only handoff file degrades to a clean miss (exit 3) with a best-effort delete, checked *before* validation, and never halts.
- [ ] Trace timestamps are UTC via `toISOString()` (trailing `Z`), aligning with `conductor-db`'s UTC timestamps in the shared log.
