...
**Deduplication and element filtering:** The writer (a) filters out empty strings and whitespace-only strings from all arrays, (b) deduplicates remaining elements by exact string equality, and (c) applies array and per-element caps — all before serialization; first occurrence wins for deduplication. Writer-side filtering of empty/whitespace elements prevents self-induced `SNAP_INVALID — empty element in <key>` failures. The validator does NOT enforce array element uniqueness — deduplication is the writer's responsibility only; the validator is silent on duplicate values.
