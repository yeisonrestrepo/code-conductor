...
- `scripts/snap-validate.mjs` — new file; ≤30 lines (**hard design constraint**, not a soft guideline — all required checks must fit within 30 non-blank, non-comment source lines); executed as a standalone native ES module CLI process (`node scripts/snap-validate.mjs`) — the `.mjs` extension forces ES module parsing independent of the host project's `package.json` `"type"` field; no `require()` or CommonJS globals
