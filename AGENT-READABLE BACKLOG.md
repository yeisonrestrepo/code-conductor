...
### [~] `[BUG-034]` Guard 4 Fails Open When Python Is Absent, Silently Ceasing To Guard (SUPERSEDED by `[BUG-036]`)
...
* **Description:** Four independent defects, of which `[BUG-034]` is the fourth. (1) Both `settings.json` files wire `PreToolUse` with `"matcher": "Write|Edit|create_file|write_file"`, so the hook is never invoked for a `Read` and Guards 1 and 4 are unreachable. (2) Per the hook reference read 2026-09-25, a `PreToolUse` hook receives `{"tool_name","tool_input",...}` as JSON on stdin; the script reads `CLAUDE_TOOL_NAME` / `CLAUDE_TOOL_INPUT` from the environment, and Guard 4 dereferences them without the `${VAR:-}` default the other guards use, so it aborts under `set -u`. (3) `exit 1` is a non-blocking error; only exit 2 blocks, or `hookSpecificOutput.permissionDecision: "deny"`. Guard 4 prints the legacy `{"decision":"block"}` and exits 1, which is neither. (4) Guard 4's path check shells out to `python3 -c` with `|| true`, so a Python-free host allows the read.
* **Impact:** A guardrail product whose guards demonstrably never fire. `README.md` and both `CLAUDE.md` files state that Guard 4 blocks reads at the hook level; that claim is false on every platform. The 125 guard tests pass because they invoke the script directly with synthetic env vars against the repository-local copy, so the suite proves the matching logic and has never touched the wiring, the input shape or the blocking mechanism - the `[BUG-033]` shipped-asset blind spot one layer up.
* **History is "never verified", not drift:** the reference documents no environment variables for hook input, not even as deprecated, and carries no version history or migration note for the `PreToolUse` contract. It does annotate version floors where they exist (v2.1.267 for an unrelated `StopFailure` matcher), so the absence of one here is informative. No artifact in this repo has ever exercised the real contract. Local Claude Code is 2.1.282.
* **Components Affected:** new `project-template/.claude/hooks/pre-tool-use.mjs` and its `.claude/hooks/` mirror, both `settings.json` files, `project-template/.claude/hooks/pre-tool-use.sh` (deleted), new `tests/hooks/pre-tool-use-contract.test.js`, `tests/hooks/guard4.test.js`, `tests/installer/templates.test.js`, `README.md`, `CLAUDE.md`, `project-template/CLAUDE.md`.
* **Acceptance Criteria:** A single Node front door parses stdin once, dispatches on `tool_name` through a four-wide table, and denies only via `hookSpecificOutput.permissionDecision` with exit 0. Guards 1, 2 and 4 port with their logic unchanged; Guard 2 becomes `"ask"` because that is its existing prompt semantics in the platform's vocabulary; Guard 3's slot is declared and empty. Fail policy: unparseable stdin denies pre-dispatch with a `CC_HOOK_ALLOW=1` override and one stderr line, a parsed payload whose guard has no field to act on allows, and a valid-but-unfamiliar payload allows. An integration harness invokes the hook exactly as Claude Code does and every guard's cases run through it. A parity test pins the mirrored pair. Docs state the shipped truth. `[BUG-034]` is marked superseded, not done. Spec: `docs/superpowers/specs/2026-09-25-bug036-pre-tool-use-hook-contract-design.md`.

### [X] `[BUG-037]` Guard 3 Has Never Shipped: Port the Bash Command Scanner and Deliver It
* **Description:** `.claude/hooks/pre-tool-use.sh` in this repository is 519 lines and carries Guard 3, the bash command scanner with twelve patterns and the `BASH_SCAN_ALLOWLIST` family. `project-template/.claude/hooks/pre-tool-use.sh`, the file the installer deploys, is 81 lines and contains zero occurrences of Guard 3. The scanner has reached no user. The 108 `guard3.test.js` cases target the repository-local copy, so they verify a file that ships to nobody, and no parity test covers the pair as the `cc-plan.md` mirrors are covered.
* **Impact:** An advertised guard that does not exist on any installed machine. Unlike Guards 1 and 4, which are present but unreachable, this one is absent from the artifact entirely, so `[BUG-036]`'s contract repair cannot deliver it.
* **Components Affected:** `project-template/.claude/hooks/pre-tool-use.mjs` (Guard 3's declared slot), `tests/fixtures/guard3-reference.sh`, `tests/hooks/guard3.test.js`, `tests/guard3-test.sh`, `README.md`.
* **Acceptance Criteria:** The twelve POSIX ERE patterns are ported to JavaScript RegExp and kept as a visible table (data, not scattered code). The entire existing 108-case corpus runs against the ported patterns unchanged, plus added cases for known ERE-to-RegExp dialect divergences; any pattern that cannot be translated with proven equivalence becomes its own line item with its own red state rather than a silent approximation. `BASH_SCAN_ALLOWLIST` semantics are preserved. Decide in that spec whether a `CC_GUARD3_WARN=1` soft mode ships for the first user cohort, since this is a first-ever delivery and real developer commands will meet twelve block patterns for the first time. `README.md` moves from "ships in `[BUG-037]`" to describing the shipped behavior.

### [ ] `[BUG-038]` /cc-checkpoint Carries Forward a Phase Only /cc-compact Writes, So the Recorded Phase Goes Stale
* **Description:** `/cc-checkpoint`'s fail-open DB tail derives `ph` by carry-forward: it runs `conductor-db get-session "$id"` and reuses the stored `phase` when it is a non-empty enum member, defaulting to `"impl"` only on no row, empty phase, DB unavailable or timeout. But `/cc-compact` is the only command that writes a session row. `/cc-plan` and `/cc-implement` write task rows through the `record` subcommand and never touch `sessions`, so any checkpoint taken after a plan and an implementation reuses the phase from the last compaction, which by then is two phases old.
* **Reproduction (observed 2026-09-25, BUG-036 cycle):** `/cc-compact` at the spec boundary wrote `phase=spec`. `/cc-plan` and `/cc-implement` then ran to completion across six commits. `/cc-checkpoint` at the release boundary read `phase=spec` from the carry-forward and would have written a v2 snapshot claiming the session was still in its spec phase, keyed to the release commit. The value written was `impl` instead, the skill's own no-row default, and the deviation was flagged rather than silently recorded.
* **Impact:** `resume-read.mjs` prefers the DB snapshot over the handoff file and prints `phase: <ph>` in its `RESUME_HIT` block, which every phase-entry command echoes to the user as `> Resumed from stored snapshot @ <commit> (phase: <phase>)`. A stale phase therefore tells the next session it is resuming work it already finished. The blob is otherwise correct, so nothing fails loudly; the record is simply wrong, which is the failure shape `[BUG-031]` and `[BUG-036]` both had.
* **Components Affected:** `global/commands/cc-checkpoint.md:36` (the carry-forward step; the deployed `~/.claude/commands/cc-checkpoint.md` is byte-identical and there is no `project-template/` mirror), `scripts/conductor-db.mjs` (`session` / `get-session`), `scripts/resume-read.mjs` (the consumer that surfaces the value), `.claude/commands/cc-plan.md` and `.claude/commands/cc-implement.md` plus their `project-template/` mirrors if the fix makes them phase-aware.
* **Candidate shape, not a decision:** have `/cc-checkpoint` write its own session row with a phase it determines, rather than deriving one only `/cc-compact` produces. Whether the phase is inferred, passed as an argument, or written by `/cc-plan` and `/cc-implement` at their own boundaries belongs to this item's spec.
* **Acceptance Criteria:** A checkpoint taken after a plan and an implementation records the phase that actually just completed, not the one the last compaction left behind. The carry-forward's staleness window is closed or documented as bounded, the DB tail stays fail-open with no new hard dependency, and a test pins the recorded phase across a spec-then-plan-then-implement-then-checkpoint sequence.

### [ ] `[BUG-039]` deployProject Force-Copies the Template Over Host-Owned Project Files, Destroying Shared Memory
* **Description:** `lib/installer/deploy.mjs:152` copies `project-template/.claude` into the host's `.claude/` with `CP_OPTS` (`{ recursive: true, force: true, dereference: false }`) and no filter, so every `code-conductor --project` re-run overwrites whatever the host owns at a path the template also ships. Two files are affected today: `.claude/memory/project.md`, the team's shared decision log, is replaced by the template stub, and `.claude/memory/context-threshold.txt` reverts to the default, discarding an operator's tuned value.
* **The fix already exists on the other half of the installer.** `deployGlobal` solves exactly this at `lib/installer/deploy.mjs:49` with `skipHostOwned`, a `cpSync` filter that excludes `global/memory` from the copy so `personal.md` and `verbosity.md` survive, and then creates the directory explicitly (`deploy.mjs:119`) so write-if-absent seeding still works. The project half was never given the equivalent.
* **Impact:** The product's own memory architecture calls `project.md` the source of truth that is committed and shared with the team; `CLAUDE.md` instructs every session to read it first. An installer re-run silently replaces it with a stub, and the loss is invisible until a session reads the stub and behaves as though the project has no recorded decisions. `1.28.0`'s changelog actively instructs users to re-run the installer, which makes the trigger more likely to be pulled, and `1.29.0` will repeat that instruction.
* **Same defect family as `[BUG-035]`:** installer force-copy versus host-owned files. `[BUG-035]` is the `settings.json` case, where the force-copy destroys user-added `UserPromptSubmit` entries the mergers are built to preserve; this is the project-memory case. The queue decision after `1.29.0` should weigh fixing them as one spec, since both are the same missing distinction between what the installer owns and what the host owns.
* **Components Affected:** `lib/installer/deploy.mjs` (`deployProject` at :152, `skipHostOwned` at :49, `CP_OPTS` at :23), `project-template/.claude/memory/project.md` and `context-threshold.txt` (the shipped files whose paths collide), `tests/installer/deploy.test.js`, `tests/installer/cli.test.js` (the end-to-end re-run case).
* **Candidate shape, not a decision:** give `deployProject` the same `skipHostOwned`-style filter `deployGlobal` already uses, so one fix covers both files, then seed the excluded paths write-if-absent so a fresh scaffold still gets the template's `project.md`. Whether the seeding is shared with `deployGlobal`'s or duplicated, and whether `[BUG-035]`'s `settings.json` case folds into the same filter, belongs to that spec.
* **Acceptance Criteria:** A `--project` re-run over an existing installation leaves a host-modified `.claude/memory/project.md` and `.claude/memory/context-threshold.txt` byte-identical, while a fresh scaffold still receives both from the template. A test covers the re-run case specifically, since the defect is invisible on first install and only appears on the second.
