...
// CLAUDE.md and .gitignore are host-authored data, not managed assets: they are
// merged, never force-copied. Everything else in the template keeps its overwrite.
const MERGED_ROOT_FILES = new Set(['CLAUDE.md', '.gitignore']);
...
export function deployProject(assetRoot, cwd) {
  ...
  mergeFileInto(join(templateRoot, 'CLAUDE.md'), join(cwd, 'CLAUDE.md'), mergeClaudeMdText);
  mergeFileInto(join(templateRoot, '.gitignore'), join(cwd, '.gitignore'), appendMissingLinesText);
