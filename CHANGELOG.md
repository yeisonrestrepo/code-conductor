...
- Bundled skills were copied to `~/.claude/skills/<name>.md`, a path Claude Code never reads, so `critical-review`, `code-simplifier`, `verbosity`, `memory-first` and `agent-delegation` silently failed to register and every command that invokes them skipped the protocol. They now ship and install as `skills/<name>/SKILL.md`; `code-simplifier` and `verbosity` gained the `name`/`description` frontmatter they were missing; a file blocking `skills/<name>` is removed before the copy, and a stale flat `<name>.md` from an older install is swept when it still matches the bundled content.
